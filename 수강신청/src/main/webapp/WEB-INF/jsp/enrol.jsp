...
<jsp:include page="test.jsp" flush="false" />
   <c:choose>
      <c:when test="${result eq true }">
         <script>alert("신청 되었습니다.");</script>
      </c:when>
      <c:when test="${result eq false }">
         <script>alert("신청내역이 존재합니다.");</script>
      </c:when>
   </c:choose>
   <div class="wrap">
   <table style="width:100%">
    <caption class="abc">수강신청</caption>
      <thead>
         <tr>
            ...
            <tr>
               ...
               <td bgcolor="#efefef">
                  <form action="enrol" method="post">
                     ...
                     <input type="hidden" name="time" value=${lecture.time }/>
                     <input type="submit" style="font-size:12pt; color:#fff; font-weight:bold;" value="수강 신청"/>
                  </form>
               </td>
